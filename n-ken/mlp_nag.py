...
if __name__ == '__main__':
    ...
    for loop in tqdm(range(400)):
        ...
        for i in range(0, X_train.shape[0], batch_size):
            ...
            # delta * momentumを加算
            layer1._w += delta_w1 * momentum
            layer1._b += delta_b1 * momentum
            layer2._w += delta_w2 * momentum
            layer2._b += delta_b2 * momentum
    ...
    plt.title("テストセットでのAccuracy(Nesterov)")
    plt.ylim(0, 1)
    plt.xlabel("反復回数")
    plt.ylabel("精度")
    plt.ylim(0, 1)
    plt.savefig("accuracy_on_test_nag.png", transparent=True)
