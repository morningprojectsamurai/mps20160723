...
if __name__ == '__main__':
    ...
    for loop in tqdm(range(400)):
        ...
        for i in range(0, X_train.shape[0], batch_size):
            ...
            layer1._w -= lr / np.sqrt(g_w1) * grad_w1
            layer1._b -= lr / np.sqrt(g_b1) * grad_b1
            layer2._w -= lr / np.sqrt(g_w2) * grad_w2
            layer2._b -= lr / np.sqrt(g_b2) * grad_b2
    ...
    plt.title("テストセットでのAccuracy(Adagrad)")
    plt.xlabel("反復回数")
    plt.ylabel("精度")
    plt.ylim(0, 1)
    plt.savefig("accuracy_on_test_adagrad.png", transparent=True)
