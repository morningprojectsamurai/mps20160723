...
if __name__ == '__main__':
    ...
    for loop in tqdm(range(400)):
        ...
        for i in range(0, X_train.shape[0], batch_size):
            ...
            # パラメーターの更新
            m_w1 = gamma * m_w1 + (1 - gamma) * grad_w1
            m_b1 = gamma * m_b1 + (1 - gamma) * grad_b1
            m_w2 = gamma * m_w2 + (1 - gamma) * grad_w2
            m_b2 = gamma * m_b2 + (1 - gamma) * grad_b2

            n_w1 = gamma * n_w1 + (1 - gamma) * grad_w1 ** 2
            n_b1 = gamma * n_b1 + (1 - gamma) * grad_b1 ** 2
            n_w2 = gamma * n_w2 + (1 - gamma) * grad_w2 ** 2
            n_b2 = gamma * n_b2 + (1 - gamma) * grad_b2 ** 2

            delta_w1 =  mu * delta_w1 - lr / (np.sqrt(n_w1 - m_w1**2 + eps)) * grad_w1
            delta_b1 =  mu * delta_b1 - lr / (np.sqrt(n_b1 - m_b1**2 + eps)) * grad_b1
            delta_w2 =  mu * delta_w2 - lr / (np.sqrt(n_w2 - m_w2**2 + eps)) * grad_w2
            delta_b2 =  mu * delta_b2 - lr / (np.sqrt(n_b2 - m_b2**2 + eps)) * grad_b2
    ...
    plt.title("テストセットでのAccuracy(RMSPropGraves)")
    plt.xlabel("反復回数")
    plt.ylabel("精度")
    plt.ylim(0, 1)
    plt.savefig("accuracy_on_test_rmspropgraves.png", transparent=True)
